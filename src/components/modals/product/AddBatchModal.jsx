...
const AddBatchWrapperModal = ({ productId, onClose, onSuccess, editData, onBatchUpdated }) => {
  ...
  const handleSetPrice = async () => {
    ...
    if (!parsedPrice || !productId) {
      setSubmissionError("Please enter a valid price.");
      return;
    }
    ...
    try {
      await instance.put("product/price", {
        productId,
        price: parsedPrice,
      });

      setSubmissionSuccess("Price set successfully!");
        onSuccess?.();
      setTimeout(() => {
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl p-6 w-[90%] max-w-[480px] text-left shadow-lg">
            ...
            <div className="space-y-2 text-sm">
              ...
              <SummaryRow label="Average Selling Price" value={priceSuggestion.batchStatistics?.averageSellingPrice ? `₦${priceSuggestion.batchStatistics?.averageSellingPrice.toFixed(2)}` : '-'} />
            ...
            <div className="mt-4">
              <Input label="Set New Price" name="newPrice" placeholder="Enter new price" value={newPrice} onChange={(e) => setNewPrice(e.target.value)} />
            </div>

            {submissionError && <p className="text-red-500 text-xs mt-1">{submissionError}</p>}
            {submissionSuccess && <p className="text-green-500 text-xs mt-1">{submissionSuccess}</p>}

            <div className="flex space-x-2 mt-4">
              <button
                onClick={handleSetPrice}
                disabled={submissionLoading}
                className="bg-primary_blue text-white px-6 py-2 rounded-md font-semibold w-full"
              >
                {submissionLoading ? 'Setting Price...' : 'Set Price'}
              </button>
              <button
                onClick={() => {
                  setShowPriceModal(false);
                ...
                className="bg-gray-300 text-gray-800 px-6 py-2 rounded-md font-semibold w-full"
              >
                Close
              </button>
            </div>
      ...
      {showPriceModal && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl p-6 w-[90%] max-w-[480px] shadow-lg">
            <h3 className="text-lg font-semibold mb-4">Set Product Price</h3>
            ...
            <div className="space-y-2 text-sm mb-4">
              {batches.map((batch, idx) => (
                <div
                  key={idx}
                  className="bg-[#F5F5F5] rounded p-2 flex justify-between"
                >
                  <span>BN: {batch.batchNumber}</span>
            ...
            {submissionSuccess && (
              <p className="text-green-500 text-xs mt-1">{submissionSuccess}</p>
            )}
            ...
            <div className="flex gap-2 mt-4">
              <button
                onClick={handleSetPrice}
                disabled={submissionLoading}
                className="bg-primary_blue text-white py-2 rounded-md font-semibold w-full"
              >
                {submissionLoading ? "Setting Price..." : "Set Price"}
              </button>
              ...
              <button
                ...
                className="bg-gray-300 text-gray-800 py-2 rounded-md font-semibold w-full"
              >
                Cancel
              </button>
            </div>
